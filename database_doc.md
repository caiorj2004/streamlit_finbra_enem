...
- Nome dos Conjuntos de Dados: Despesas Municipais por Função (FINBRA) e Candidatos do ENEM.

- Fonte: Data IESB (Conexão via PostgreSQL).

- Tabelas Integradas: finbra_2023, candidatos_enem, municipio, populacao, unidade_federacao.

## Contexto do Negócio

- Objetivo Principal do Projeto: Analisar e quantificar a associação entre os gastos públicos municipais em diversos setores da sociedade (Funções FINBRA) e o desempenho educacional de jovens (15-19 anos) no Exame Nacional do Ensino Médio (ENEM).
...
- Pivotagem: A base FINBRA (originalmente em formato Long, com uma linha por função) foi pivotada para o formato Wide, onde cada função social se torna uma feature individual ([Função]_per_capita).
